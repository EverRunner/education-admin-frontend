...
<template>
  <section class="home public-body">
    ...
    <el-table :data="list" v-loading="loading" class="publi-table">
      ...
      <el-table-column align="center" prop="membercount" label="发送数量">
      </el-table-column>
...
import { getSendNoticeList } from "@api/message";
...
export default {
  ...
  methods: {
    ...
    async queryList() {
      ...
      const { data: resData } = await getSendNoticeList(
        this.params,
        this.pages
      );
    ...
    handleGoTo(row, type) {
      this.$router.push({
        name:
          type == "details"
            ? "MESSAGE_STAND_LETTER_INOF_LIST"
            : "MESSAGE_STAND_LETTER_EDIT",
        query: {
          id: row.id,
        },
      });
