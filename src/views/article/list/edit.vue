...
<template>
  <section>
    <div class="member-details-wp" style="margin-bottom: 80px">
      <div class="member-edit-title">文章编辑</div>
      ...
      <el-form :model="params" :rules="rules" ref="ruleForm" label-width="80px">
        ...
        <el-form-item label="文章缩图" prop="thumb">
          <el-upload
...
export default {
  ...
  methods: {
     ...
     */
    async queryInfo() {
      const { data: resData } = await getArticleInfo(this.params.id);
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.$confirm("是否保存？", "提示", {
          ...
          .then(async () => {
            if (this.params.id) {
              const { data: resData } = await eidtArticle(this.params);
              if (!resData.data.status) return;

              this.$router.go(-1);
              this.$message.success("修改成功");
            } else {
              const { data: resData } = await addArticle(this.params);
              if (!resData.data.status) return;
